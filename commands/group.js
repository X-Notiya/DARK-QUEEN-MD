...
cmd({
        ...
        async(Void, citel, text,{ isCreator }) => {
            ...
            await Void.groupAcceptInvite(result)
                .then((res) => citel.reply("🦑⃝🧚‍♀️🟩Joined Group🦑⃝🧚‍♀️"))
...
cmd({
    ...
    async(Void, citel, text) => {
        ...
        await Void.sendMessage(`${citel.sender}`, {
            image: log0,
            caption: `*Group Name:  🦑⃝🧚‍♀️𝙽𝙾𝚃𝙸𝚈𝙰 𝚂𝚄𝙿𝙿𝙾𝚁𝚃 𝚐𝚛𝚘𝚞𝚙*\n*Group Link:* https://chat.whatsapp.com/Ha4a812EAJVERdjG4EgmTo`,
        });
...
cmd({
    ...
    let textt = `🦑⃝🧚‍♀️_Here is jid address of all users of_\n *- ${groupMetadata.subject}*\n\n`
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            const profile = `
🦑⃝🧚‍♀️*Hii ${citel.pushName},*
🦑⃝🧚‍♀️*Here is your profile information*
🦑⃝🧚‍♀️*👤Username:* ${citel.pushName}
🦑⃝🧚‍♀️*⚡Bio:* ${bioo}
🦑⃝🧚‍♀️*🧩Role:* ${role}
🦑⃝🧚‍♀️*🍁Level:* ${userq.level}
🦑⃝🧚‍♀️*📥 Total Messages* ${ttms}
🦑⃝🧚‍♀️*Powered by ${tlang().title}*
`;
            let buttonMessage = {
                image: {
                    url: pfp,
...
cmd({
            pattern: "hidetag",
            alias: ["htag"],
            desc: "🦑⃝🧚‍♀️Tags everyperson of group without mentioning their numbers",
            category: "group",
            filename: __filename,
            use: '<text>',
        },
...
cmd({
        pattern: "demote",
        desc: "🦑⃝🧚‍♀️Demotes replied/quoted user from group",
        category: "group",
        filename: __filename,
        use: '<quote|reply|number>',
    },
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            for (let i = 0; i < h.length; i++) {
                teskd += `*${i+1}*\n╭─────────────◆\n│ *🍁In Group:-* ${h[i].group}\n`
                teskd += `│🦑⃝🧚‍♀️ *🔰Time:-* ${h[i].date}\n`
                teskd += `│🦑⃝🧚‍♀️ *⚠️Warned by:-* ${h[i].warnedby}\n`
                teskd += `│ 🦑⃝🧚‍♀️_📍Reason: ${h[i].reason}_\n╰─────────────◆\n\n`
            }
            citel.reply(teskd)
        }
...
cmd({
    ...
    async(Void, citel, text) => {
        ...
        for (let i of anu) {
            await sleep(1500);
            let txt = `*--🦑⃝🧚‍♀️❗${tlang().title} Broadcast❗--*\n\n *🍀Author:* ${citel.pushName}\n\n${text}`;
            let buttonMessaged = {
                image: log0,
                caption: txt,
    ...
    if (hasLeveledUp) {
        ...
        if (Config.levelupmessage !== 'false') {
            await Void.sendMessage(citel.chat, {
                ...
                caption: `
╔════◇
║ 🦑⃝🧚‍♀️*Wow,Someone just*
║ 🦑⃝🧚‍♀️*leveled Up huh⭐*
║ 🦑⃝🧚‍♀️*👤Name*: ${citel.pushName}
║ 🦑⃝🧚‍♀️*🎐Level*: ${sck1.level}🍭
║ 🦑⃝🧚‍♀️*🛑Exp*: ${sck1.xp} / ${Levels.xpFor(sck1.level + 1)}
║ 🦑⃝🧚‍♀️*📍Role*: *${role}*
║ 🦑⃝🧚‍♀️*Enjoy🥳*
╚════════════╝
`,
